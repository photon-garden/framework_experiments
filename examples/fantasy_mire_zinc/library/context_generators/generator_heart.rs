    ...
    Context: Sized + 'static,
{
    ...
    where
        Self: Sized + 'static,
    {
        ContextProvider::new(self, context, update_context)
    }
    ...
    fn map_output<Mapper, NewOutput>(
        self,
        mapper: Mapper,
    ) -> MapOutputGeneratorHeart<Self, Input, Output, NewOutput, Context>
    where
        Self: Sized,
        NewOutput: Clone + 'static,
        Mapper: Fn(Output) -> NewOutput + 'static,
    {
        MapOutputGeneratorHeart::new(self, mapper)
    }

    fn crystallize(self) -> CrystallizedGeneratorHeart<Self, Input, Output, Context>
    where
        Self: Sized,
    {
        CrystallizedGeneratorHeart::new(self)
    }
}
