...
pub type HslGenerator = BoxedGenerator<Hsl>;

pub trait IntoBoxedColorGenerator {
    fn into_boxed_color_generator(self) -> HslGenerator;
}
...
impl Generator for LoopedColorGenerator {
    type Output = Hsl;
...
impl LoopedColorGenerator {
    pub fn color_picker(mut self, red: u8, green: u8, blue: u8, alpha: u8) -> LoopedColorGenerator {
        let new_color = Colors::color_picker(red, green, blue, alpha);
        self.colors.push(new_color);
        self
    }
...
pub fn repeated_colors() -> LoopedColorGenerator {
